...
CA_CERT=$($(get_kubectl_cmd mgmt) -n vault exec -i vault-0 -- sh -c '
  export VAULT_TOKEN="'"${VAULT_ROOT_TOKEN}"'"
  vault read -field=certificate pki/cert/ca 2>/dev/null || true
')
...
$(get_kubectl_cmd mgmt) -n vault exec -i vault-0 -- sh -c '
  export VAULT_TOKEN="'"${VAULT_ROOT_TOKEN}"'"
  vault write pki/roles/istio-gateway \
...
$(get_kubectl_cmd mgmt) -n vault exec -i vault-0 -- sh -c '
  export VAULT_TOKEN="'"${VAULT_ROOT_TOKEN}"'"
  vault policy write cert-manager-pki - <<POLICY
# PKI 인증서 발급 권한
...
AUTH_ENABLED=$($(get_kubectl_cmd mgmt) -n vault exec -i vault-0 -- sh -c '
  export VAULT_TOKEN="'"${VAULT_ROOT_TOKEN}"'"
  vault auth list -format=json 2>/dev/null | jq -r ".\"kubernetes/\"" || echo "null"
')
...
CA_CERT_B64=$($(get_kubectl_cmd mgmt) -n vault exec -i vault-0 -- sh -c '
  export VAULT_TOKEN="'"${VAULT_ROOT_TOKEN}"'"
  vault read -field=certificate pki/cert/ca | base64 -w0
')
